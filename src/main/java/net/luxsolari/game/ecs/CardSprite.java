...
public final class CardSprite implements Component {
  ...
  public CardSprite(String[] faceUp, String[] back, boolean faceUpInitial) {
    ...
    for (String row : faceUp)
      if (row.length() != w) throw new IllegalArgumentException("Sprite rows must be equal length");
    for (String row : back)
      if (row.length() != w) throw new IllegalArgumentException("Sprite rows must have same width");
    this.faceUp = faceUp;
    this.back = back;
